...
int InvChebyPoly(int NumPoles, double StopBanddB, CplxD *ChebyPoles, CplxD *ChebyZeros, int *ZeroCount)
{
 ...
 for(j=0; j<=2*N; j++)SquaredPolyCoeff[j] = CplxD(0.0,0.0);
 ...
 // Numerator
 k=0;
 for(j=0; j<=2*N; j++)ChebPolyCoeff[j] = SquaredPolyCoeff[j].re;  // Not using Epsilon here so the check for 0 on the next line is easier. Since the root finder normalizes the poly, it gets factored out anyway.
 for(j=0; j<=2*N; j++)if( fabs(ChebPolyCoeff[j]) > 0.01)break;    // Get rid of the high order zeros. There will be eithe 0ne or two zeros to delete.
 for(   ; j<=2*N; j++)PolyCoeff[k++] = ChebPolyCoeff[j];
 k--;
 ...
 for(j=0; j<MAX_ELLIP_ITER; j++)
  {
   ...
   K[0] -= DeltaK;
   if(K[0] < 0.001)K[0] = 0.001;  // must not be < 0.0
   if(K[0] > 0.990)K[0] = 0.990;  // if > 0.990 we get a pole in the RHP. This means we were unable to set the stop band atten to the desired level (the Ripple is too large for the Pole Count).
  }
