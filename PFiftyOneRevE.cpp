...
/*
 ...
 to search for complex pairs in the arrays.
...
{
 int j;
 long double  P[P51_ARRAY_SIZE], RealRoot[P51_ARRAY_SIZE], ImagRoot[P51_ARRAY_SIZE];

 for(j=0; j<=N; j++)P[j] = Coeff[j];      // double to long double
 N = PFiftyOne(P, N, RealRoot, ImagRoot);
 for(j=0; j<N; j++)Roots[j] = CplxD((double)RealRoot[j], (double)ImagRoot[j]); // long double to double
 return(N);
}
...
void UpdateTUV(int Iter, long double *P, int N, long double *QP, long double *K, long double *QK, long double *TUV, TUpdateStatus *UpdateStatus)
{
 ...
 if( fabsl(DelU) < LDBL_EPSILON * fabsl(TUV[1]) )
  {
   if(DelU < 0.0)DelU = -fabsl(TUV[1]) * LDBL_EPSILON;
   else          DelU =  fabsl(TUV[1]) * LDBL_EPSILON;
  }

 DelV = -E5*K[N-2]*TUV[2]*TUV[2] / Denom;
 if( fabsl(DelV) < LDBL_EPSILON * fabsl(TUV[2]) )
  {
   if(DelV < 0.0)DelV = -fabsl(TUV[2]) * LDBL_EPSILON;
   else          DelV =  fabsl(TUV[2]) * LDBL_EPSILON;
  }
 ...
 if(Iter >= QUAD_ITER_MAX-1)
  {
   ...
   if(fabsl(DelU) < 8.0 * LDBL_EPSILON * fabsl(TUV[1]) && fabsl(DelV) < 8.0 * LDBL_EPSILON * fabsl(TUV[2]) )
    {
     *UpdateStatus = ZERO_DEL;
     return;
    }
   // A big change after this many iterations means we are wasting our time on this angle.
   if(fabsl(DelU) > 10.0*fabsl(TUV[1]) || fabsl(DelV) > 10.0*fabsl(TUV[2]) )
    {
     *UpdateStatus = BAD_ANGLE;
     return;
    }
 ...
 // Update U and V
 TUV[1] += DelU;
 if( fabsl(TUV[2] + DelV) < TINY_VALUE )DelV *= 0.9;  // If this, we would set TUV[2] = 0 which we can't allow, so we use 90% of DelV.
 TUV[2] += DelV;
 ...
 for(Iter=0; Iter<REAL_ITER_MAX; Iter++)
  {
   ...
   else if(Err > HUGE_VALUE)return(0);  // Overflow is imminent.
   ...
   if( fabsl(QK[N-1]) <= TINY_VALUE * fabsl(P[N]) )return(0);  // QK[N-1] ~ 0 will cause overflow below.
   ...
   if(X != 0.0)
    {
     if(fabsl(DelX) < LDBL_EPSILON * fabsl(X))  // If true, the algorithm is stalled, so bump X by 2 epsilons.
      {
       if(DelX < 0.0)DelX = -2.0 * X * LDBL_EPSILON;
       else          DelX =  2.0 * X * LDBL_EPSILON;
      }
    }
   else  // X = 0
    {
     if(DelX == 0.0)return(0); // Stalled at the origin, so exit.
    }
   X -= DelX;  // Update X
...
const long double Angle[] =
 ...
 if(TypeOfQuadK == 0)  // Init QuadK 2nd derivative of P
  ...
  {
   for(j=0; j<=N-2; j++)QuadK[j] = RealK[j+1];
   QuadK[N] = QuadK[N-1] = 0.0;
  }

 else  // Set QuadK to zero, except QuadK[0].
  {
   for(j=1; j<=N; j++)QuadK[j] = 0.0;
   QuadK[0] = 1.0;
  }
