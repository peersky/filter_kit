...
TSPlaneCoeff CalcLowPassProtoCoeff(TLowPassParams Filt)
{
  ...
  if(Filt.NumPoles < 1)Filt.NumPoles = 1;
  if(Filt.NumPoles > MAX_POLE_COUNT)Filt.NumPoles = MAX_POLE_COUNT;
  if(Filt.ProtoType == ELLIPTIC || Filt.ProtoType == INVERSE_CHEBY)
   {
    if(Filt.NumPoles > 15)Filt.NumPoles = 15;
   }
  if(Filt.ProtoType == GAUSSIAN || Filt.ProtoType == BESSEL)
   {
    if(Filt.NumPoles > 12)Filt.NumPoles = 12;
   }

  // Gamma is used by the Adjustable Gauss.
  if(Filt.Gamma < -1.0)Filt.Gamma = -1.0; // -1 gives ~ Gauss response
  if(Filt.Gamma > 1.0)Filt.Gamma = 1.0;   // +1 gives ~ Butterworth response.

  // Ripple is used by the Chebyshev and Elliptic
  if(Filt.Ripple < 0.0001)Filt.Ripple = 0.0001;
  if(Filt.Ripple > 1.0)Filt.Ripple = 1.0;
  ...
  if(Filt.ProtoType == CHEBYSHEV && Filt.NumPoles > 15)
   {
    ...
    if(Filt.NumPoles == 16)MaxRipple = 0.5;
    if(Filt.NumPoles == 17)MaxRipple = 0.4;
    if(Filt.NumPoles == 18)MaxRipple = 0.25;
    if(Filt.NumPoles == 19)MaxRipple = 0.125;
    if(Filt.NumPoles >= 20)MaxRipple = 0.10;
    if(Filt.Ripple > MaxRipple)Filt.Ripple = MaxRipple;
   }

  // StopBanddB is used by the Inverse Chebyshev and the Elliptic
  // It is given in positive dB values.
  if(Filt.StopBanddB < 20.0)Filt.StopBanddB = 20.0;
  if(Filt.StopBanddB > 120.0)Filt.StopBanddB = 120.0;
  ...
  if(Filt.NumPoles == 1)
   ...
   {
	NumRoots   = InvChebyPoly(Filt.NumPoles, Filt.StopBanddB, Poles, Zeros, &ZeroCount);
	DenomCount = GetFilterCoeff(NumRoots, Poles, Coeff.D2, Coeff.D1, Coeff.D0);
   ...
   {
	NumRoots   = EllipticPoly(Filt.NumPoles, Filt.Ripple, Filt.StopBanddB, Poles, Zeros, &ZeroCount);
	DenomCount = GetFilterCoeff(NumRoots, Poles, Coeff.D2, Coeff.D1, Coeff.D0);
	NumerCount = GetFilterCoeff(ZeroCount, Zeros, Coeff.N2, Coeff.N1, Coeff.N0);
	SetCornerFreq(DenomCount, Coeff.D2, Coeff.D1, Coeff.D0, Coeff.N2, Coeff.N1, Coeff.N0);
   }
...
void SetCornerFreq(int PolyCount, double *D2, double *D1, double *D0, double *N2, double *N1, double *N0)
{
 int j, n;
 ...
 Gain = 1.0;
 for(j=0; j<PolyCount; j++)Gain *= D0[j] / N0[j];
 ...
 for(j=0; j<PolyCount; j++)
  {
   Omega = sqrt(D0[j]);
   if(Omega == 0.0)continue;   // should never happen
   Zeta = D1[j] / Omega / 2.0;
...
int GetFilterCoeff(int RootCount, CplxD *Roots, double *A2, double *A1, double *A0)
{
 ...
 for(j=0; j<RootCount-1; j++)
  {
   for(k=j+1; k<RootCount; k++)
    {
     if( fabs(Roots[j].re - Roots[k].re) < 1.0E-3 &&
         fabs(Roots[j].im - Roots[k].im) < 1.0E-3 )
      {
 ...
 for(j=0; j<RootCount; j++)
  {
   if(Roots[j].re > 0.0)continue; // Right Hand Plane
   if(Roots[j].re == 0.0 && Roots[j].im == 0.0)continue; // At the origin.  This should never happen.
...
int RebuildPoly(int PolyCount, double *PolyCoeff, double *A2, double *A1, double *A0 )
{
 ...
 for(j=0; j<=2*PolyCount; j++)PolyCoeff[j] = 0.0;
 for(j=0; j<P51_ARRAY_SIZE; j++)Sum[j] = 0.0;
 ...
 for(j=1; j<PolyCount; j++)
  {
   ...
   for(k=0; k<=2*j+2; k++)PolyCoeff[k] = Sum[k];
   for(k=0; k<P51_ARRAY_SIZE; k++)Sum[k] = 0.0;
  }
 ...
 for(j=2*PolyCount; j>=0; j--)if(PolyCoeff[j] != 0.0)break;
 return(j);
}
...
void SortRootsByZeta(CplxD *Roots, int Count, TOurSortTypes SortType)
{
 ...
 for(j=0; j<Count; j++)
  {
   if(fabs(Roots[j].re) * 1.0E3 < fabs(Roots[j].im) )Roots[j].re = 0.0;
   if(fabs(Roots[j].im) * 1.0E3 < fabs(Roots[j].re) )Roots[j].im = 0.0;
  }

 // Sort the roots.
 for(j=0; j<Count; j++)RootJ[j] = j;  // Needed for HeapIndexSort
 if( Roots[0].re != 0.0 ) // Cplx roots
  {
   for(j=0; j<Count; j++)SortValue[j] = Roots[j].re;
  }
 else  // Imag roots, so we sort on imag part.
  {
   for(j=0; j<Count; j++)SortValue[j] = fabs(Roots[j].im);
  }
 ...
 for(FailSafe=0; FailSafe<NSquared; FailSafe++) // typical FailSafe value on return is N*log2(N)
  {
   ...
   while(j < k + 2)
	{
	 FailSafe++;
	 if(j <= k && Data[Index[j-1]] > Data[Index[j]]) j++;
